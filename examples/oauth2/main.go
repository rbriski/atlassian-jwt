...
func main() {
	...
	http.HandleFunc("/enabled", enabled(config))
...
func enabled(c *jwt.Config) func(w http.ResponseWriter, r *http.Request) {
	return func(w http.ResponseWriter, r *http.Request) {
		body, err := ioutil.ReadAll(r.Body)
		if err != nil {
			log.Fatalf("Can't read request:%v\n", err)
...
func handleIssueEvent(c *jwt.Config) func(w http.ResponseWriter, r *http.Request) {
	return func(w http.ResponseWriter, r *http.Request) {
		...
		jiraClient, _ := jira.NewClient(c.Client(ctx), c.BaseURL)
